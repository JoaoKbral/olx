...
const Page = () => {
  ...
  useEffect(() => {
    ...
    const getStates = async () => {
      const stateList = await api.getState();
  ...
  const setSubmit = async (e) => {
    ...
    const json = await api.updateUser(name, stateLoc, email, password);
    if (json.error) {
      setError(json.error);
    } else {
        ...
        <div className="list">
          {adList.map((i, k) => (
            <div className="card-ad" key={k}>
              <img src={i.images} alt="ad" width={300} height={300} />
              <div className="area-ad">
                <label className="area--title-ad">{i.title}</label>
                <br />
                <label className="area--text-ad">{i.price}</label>
